package _04_UnderstandingJavaArrays;

public class _12_CreatingArrayOfPrimitives {
    public static void main(String[] args) {

        //Ways of creating an array
        int[] numbers = new int[3]; //size
        int[] numbers1 = new int[]{ 3,5,6}; //Specify all the elements it should start out with
        int[] numbers2 = {8,7,9}; //shortcut of the above

        // you can type the [] before or after the name, and adding a space is
        //optional. This means that all five of these statements do the exact same
        //thing:
        int[] numAnimals;
        int [] numAnimals1;
        int []numAnimals2;
        int numAnimals3[];
        int numAnimals4 [];

        //multiples Arrays in declarations
        int[] ids, types; // ids[] and types[]
        int ids1[], types2; //In this case we get ids1[] and int types2
    }




}
